...
void AFPSGameMode::CompleteMission(APawn* InstigatorPawn, bool bMissinoSuccess)
{
	if (InstigatorPawn)
	{
	...
	if (GS)
	{
		GS->MulticastOnMissionComplete(InstigatorPawn, bMissinoSuccess);
	}

	OnMissionCompleted(InstigatorPawn, bMissinoSuccess);
